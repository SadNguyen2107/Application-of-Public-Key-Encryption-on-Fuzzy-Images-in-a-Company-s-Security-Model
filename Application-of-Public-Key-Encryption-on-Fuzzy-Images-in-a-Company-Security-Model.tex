...
\begin{enumerate}[label=(\roman*), itemsep=1em]
    ...
    \item How to set the open key: \\
          1 open key setting \( \mathbf{F} \) includes \( ((d, \, X), \, t, \, \mathbf{X}  ,\, \varPhi,\, \mathbf{\epsilon} ) \) with \( (d, \, X) \) being the \allowbreak spatial data with \( X \) being the space containing the values of the fuzzy set of the \allowbreak picture \( A \) and \( d :\mathbf{X^2} \to \mathbb{R} \) being the corresponding distance function. \( t \in \mathbb{R} \) is the \allowbreak threshold value determined by the security parameter \( \lambda \), \( \mathbf{X} \) is the \allowbreak distribution of the fuzzy data on \( X \), \( \varPhi \) is the error distribution and \( \mathbf{ \epsilon \in [0,1] } \) is an error \allowbreak parameter representing the false rejection rate. The False Acceptance Rate (FAR) and False Rejection Rate (FRR) are determined based on the threshold value \( t \).\\[6pt]
          \textbf{Requirement}: \( \text{FAR} := \Pr[x,\, x' \leftarrow_{\mathbf{R}} {\mathbf{X}} : d(x,\, x') < t] \) is negligible in the \allowbreak security parameter  \( \lambda \). Also for all fuzzy data parts of \( x \in \mathbf{X} \), \( \text{FRR} := \Pr[e \leftarrow_{\mathbf{R}} \varPhi : \mathrm{d}(x, \, x + e) \geq t] \leq \epsilon \)
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Membership functions applied to high level of security of fuzzy sets of \allowbreak pictures}
...
We say that the public key encryption scheme of a fuzzy picture with the fuzzy key setting \( F \) is correct, meaning that for every security parameter \( \lambda \in \mathbb{N} \) with all fuzzy picture data of \( x, x' \in X \) such that \( \mathrm{d}(x, x') < t \), for all messages \( M \) in the \allowbreak message space, if:
\begin{align*}
...
Assume that \( S = (\text{S.Setup},\, \text{S.Sketch}, \text{S.DiffRec}) \) is the probabilistic algorithm for installing the fuzzy key \( F \), and \( \text{Sig} = (\text{Sig.KeyGen},\, \text{Sig.Sign},\, \text{Sig.Verify}) \) is the \allowbreak one-time signature algorithm.
...
\begin{enumerate}
    ...
    \item \textbf{Installation}: The security parameter \( \lambda \) is taken as input. It defines the \break installation of the fuzzy key \( F = ((d,\, X),\, t,\, \mathcal{X},\, \varPhi,\, \epsilon) \). The public parameter \(  par \) is \allowbreak obtained:
          \[
