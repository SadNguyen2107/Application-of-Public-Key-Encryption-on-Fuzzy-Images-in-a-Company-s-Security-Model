...
\begin{enumerate}
    ...
    par = 
    \seqsplit{"MFwwDQYJKoZIhvcNAQEBBQADSwAwSAJBAJQVu6lHAEtia3xc8fCEKd9dpt0jGt7FSiM\ z"}
    ...
    \seqsplit{(MFwwDQYJKoZIhvcNAQEBBQADSwAwSAJBAKD/RZvqG4ocFdsCpVpUbgrlYlEumD9qe bAIVm3gv1Y6XN7w6jf2B4V9soP9jbXcmwEDy/N6xognyuqKAEB81JUCAwEAAQ==,MFwwDQYJKoZIhvcNAQEBBQADSwAwSAJBAKNQ0UmtSE2dD6Mbx0Vd8GWcTYvqPJNTqyg7xtJAYWWGPmjScKH1VUZw0lIRve3mtlLoxa7mRntUm6iw94ZSCXcCAwEAAQ).}
    ...
    =  \seqsplit{``exhxWbrXSm7huDc/4LkAHnmk3i91K1FDBqUwpk01gLR0pY8Ow/SQe3xPLpdkSpoTLwm=''}.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item With the input parameters, including the fuzzy public key, the public parameter:
        \\par = \seqsplit{``MFwwDQYJKoZIhvcNAQEBBQADSwAwSAJBAJQVu6lHAEtia3xc8fCEKd9dpt0jGt7FSiM z''},
        public key:
       \\ $pk_f$ = \seqsplit{(MFwwDQYJKoZIhvcNAQEBBQADSwAwSAJBAKD/RZvqG4ocFdsCpVpUbgrlYlEu mD9qebAIVm3gv1Y6XN7w6jf2B4V9soP9jbXcmwEDy/N6xognyuqKAEB81JUCAwEAAQ==, MFwwDQYJKoZIhvcNAQEBBQADSwAwSAJBAKNQ0UmtSE2dD6Mbx0Vd8GWcTYvqPJ NTqyg7xtJAYWWGPmjScKH1VUZw0lIRve3mtlLoxa7mRntUm6iw94ZSCXcCAwEAAQ)},
      \\ and the code\\ $CT$ = \seqsplit{``exhxWbrXSm7huDc/4LkAHnmk3i91K1FDBqUwpk01gLR0pY8Ow/SQe3xPLpdkSpoTL wm/T3KqI0qGs8HehzXHHw==''.}
...
The public-key cryptographic scheme of the fuzzy representation (PFPKE) is linked to the installation of the fuzzy key $\mathcal{F}$, including the following steps:
   ...
   \text{Step 1: Extract the fuzzy representation}. This step takes the biometric information of the user as input. The output is the fuzzy representation data:
          \[
              \text{Gen(Bio)} = \left\{ \langle x, \mu_A(x), \eta_A(x), \nu_A(x) \rangle \mid x \in X \right\}.
          \]

    \text{Step 2: Installation}. The security parameter $\lambda$ is taken as input. It defines the installation of the fuzzy key $\mathcal{F} = ((d, X), t, \mathcal{X}, \Phi, \epsilon)$. The public parameter $par$ is obtained:
          \[
   ...
   \text{Step 3: KeyGen}. Takes the public parameter $par$ and a part of the fuzzy representation data of $x$ as input. It parses $par = (par_{pke}, par_S)$, then runs:
          \[
    ...
    \text{Step 4: Encryption}. Takes the public parameters $par$, the public key $pk_f$, and the message $M$ as input. It parses $par = (par_{pke}, par_S)$ and $pk_f = (pk, c)$. The signature key generation algorithm is run:
          \[
          ...
          \[
              CT = (svk, CT, \sigma).
          \]
    \text{Step 5: Extract fuzzy representation.} This step takes the user's biometric information as input. The output is the fuzzy representation data:
         \[
          \text{Gen}(\text{Bio}) = \left\{ \langle x', \mu_A(x'), \eta_A(x'), \nu_A(x') \rangle \mid x' \in X \right\}.
         \]
    

    \text{Step 6: Decryption}. Takes the public parameters $par$, the public key $pk_f$, the fuzzy representation data of $x' \in X$, and the encoded message $CT$ as input. It parses $par = (par_{pke}, par_S)$, $pk_f = (pk, c)$, and $CT = (svk, CT, \sigma)$. If $\sigma$ is the signature on $CT$ with respect to the public key $svk$, it will run:
          \[
